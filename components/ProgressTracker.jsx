export function ProgressTracker({ progress, currentStep, status = 'processing' }) {
  ...
  const getProgressColor = () => {
    switch (status) {
      ...
      case 'processing': return 'bg-gradient-to-r from-blue-400 to-purple-500 animate-pulse-glow';
      default: return 'bg-gradient-to-r from-gray-400 to-gray-500';
    }
  };

  return (
    <div className="glass-card rounded-2xl p-6 mb-6 animate-slide-in-up">
      <div className="flex items-center justify-between mb-4">
        <h3 className="text-xl font-semibold text-gray-900">Active Batch Progress</h3>
        <span className={`text-sm font-medium ${getStatusColor()} flex items-center`}>
          {status === 'processing' && (
        ...
        <div className="grid grid-cols-5 gap-2 mt-4">
          {['Planning', 'Generating', 'Processing', 'Upscaling', 'Complete'].map((step, index) => (
            <div key={step} className="text-center">
              <div className={`w-2 h-2 rounded-full mx-auto mb-1 transition-all duration-300 ${progress >= (index + 1) * 20
